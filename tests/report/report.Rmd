...
#(read_ds("../current_performance/l0_f1_m1.opt2.csv") %>% 
#    filter(item %in% c("ms_maxrep", "total_time")) %>% mutate(value = value / 60000, label="opt"))
#(read_ds("../current_performance/l0_f1_m1.csv") %>% 
#    filter(item %in% c("ms_maxrep", "total_time"))%>% mutate(value = value / 60000, label="noopt"))

merge(filter(ds, item == "ms_maxrep") %>% select(label, value),
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
ggplot(read_ds('../use_maxrepeat/res.csv') %>% filter(measuring == "time", item == "ms_bvector"), aes(b_path, value/1000, color=label))+ geom_boxplot() + geom_point(alpha=0.3) #+ expand_limits(y = 0)
```

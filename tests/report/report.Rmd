...
#ggplot(parent_data %>% filter(b_path == "rep_100Ms_5Mt") %>% select(key, value),
#       aes(as.numeric(key), value)) + geom_line()
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height=4, fig.width=5}
drf_data <- (read_ds(stats_path, TRUE) %>% 
               filter(measuring == "double_rank_niter"))

kable(drf_data %>% 
        spread(key = key, value = value) %>% 
        select(b_path, fail, nofail) %>% 
        mutate(fail_perc = round(100 * fail / (fail + nofail), 2)) %>% 
        arrange(fail_perc), 
      caption="Double rank iterations that fail for various input types.")
...
kable(maxrep_data %>% 
        ...
        spread(key = key, value = value) %>% 
        mutate(non_maximal_perc = round(100 * non_maximal / (non_maximal + maximal), 2)),
...
#ggplot(ds %>% filter(interval_width == "same_block", method == "double_rank_fail") %>% select(ntrial, char, wl_presence, time_ms), aes(char, time_ms, color=wl_presence)) + geom_boxplot()
...
ggplot(ds, aes(interval_width, time_ms, color=method)) + 
...
ggplot(dds, aes(b_path, time_ms, color=label)) + 
  geom_boxplot(outlier.shape = NA) + 
  geom_jitter(shape=1, alpha=0.5, width=0.1, size = 3) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1), legend.position=c(.7, .8)) +
  labs(title = "Time to build the ms vector") + ylab('milliseconds')
...
ds <- (read_ds('../use_maxrepeat/sandbox/rank_timing.csv') %>% 
         ...
         select(method, wl_presence, i_width, time_ms, char, maximality) %>% filter(time_ms < 400))

#kable(spread(ds %>% group_by(method, i_width, wl_presence, maximality) %>% summarise(avg_time = mean(time_ms)), key = method, value = avg_time), digits = 2, caption = "Sandbox performance of the two tricks")

kable(spread(ds %>% 
               group_by(method, i_width, wl_presence, maximality) %>% 
               summarise(avg_time = mean(time_ms)), key = i_width, value = avg_time) %>% 
        arrange(wl_presence, maximality) %>% 
        select(wl_presence, maximality, method, different_block, same_block),
      digits = 2, caption = "Sandbox performance of the two tricks")

ggplot(ds, aes(i_width, time_ms, color=method)) + 
  geom_boxplot(outlier.shape = NA) + 
  geom_jitter(shape=1, alpha=0.5, width=0.1, size = 3) + 
  theme(axis.text.x = element_text(angle = 0, hjust = 1), legend.position="bottom") +
  facet_grid(maximality~wl_presence, scales = "free_y") +
  labs(title = "Sandbox test") + ylab('milliseconds')
```
...
## Full Algorithm Performance
```{r, echo=FALSE, warning=FALSE, message=FALSE}
ds <- read_ds('../use_maxrepeat/res.csv') %>% filter(measuring == "time") %>% filter(measuring == "time", item == "ms_bvector") %>% mutate(time_ms = 100000 * value / len_t)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
ggplot(ds %>% filter(item == "ms_bvector"), aes(b_path, time_ms, color=label)) + 
  geom_boxplot(outlier.shape = NA) + 
  geom_jitter(shape=1, alpha=0.5, width=0.1, size = 3) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1), legend.position=c(.7, .8)) +
  labs(title = "Time to build the ms vector") + ylab('milliseconds')
...
#kable(ds %>% group_by(label) %>% summarize(avg_time_sec = round(mean(value / 1000)), se = round(sd(value / 100))), caption = "Time to build the maxrep bit vector")
```

...
#input_description(1000, 3000, "mut", 100)
```

```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height=4, fig.width=5}
stats_path = "../input_stats_data/stats.csv"

parent_data <- read_ds(stats_path, TRUE) %>% filter(measuring == "consecutive_parent_calls", where=="runs") %>% mutate(b_path = inp_type)
...
p + geom_bar(mapping = aes(x = factor(as.numeric(key)), y = value), stat = "identity") + facet_wrap(~b_path, scales = "free")
rm(p, parent_data)
```
...
kable(dcast(vcomp_data %>% mutate(vector_value = ifelse(key == 1, "one", "zero")) %>% select(vector_value, value, b_path), b_path ~ vector_value, value.var="value") %>% mutate(zero_perc = round(100 * zero / one, 2)), caption="Composition of the `runs` vector for various input types.")
rm(vcomp_data)
```
...
kable(dcast(maxrep_data %>% mutate(vector_value = key) %>% select(vector_value, value, b_path), b_path~vector_value, value.var="value") %>% mutate(non_maximal_perc = round(100 * non_maximal / maximal, 2)), caption="Composition of the `B` vector (containing ends of maximal repeats) for various input types.")

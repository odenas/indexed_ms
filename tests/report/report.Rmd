...
dd <- dd %>% select(item, label, avg_time)

rank_comp <- spread(dd, key = item, value = avg_time) %>% mutate(abs_ratio = double / single, rel_ratio = 100 * abs(double - single) / single)
method_comp <- spread(dd, key = label, value = avg_time) %>% mutate(abs_ratio = FC / OD, rel_ratio = 100 * abs(FC - OD) / OD)

...
parent_data <- (read_ds(stats_path, TRUE) %>% 
                  filter(measuring == "consecutive_parent_calls", where=="runs"))
...
p + geom_bar(mapping = aes(x = factor(as.numeric(key)), y = value), stat = "identity") +
  facet_wrap(~inp_type, scales = "free")
...
```{r, echo=FALSE, message=FALSE, fig.height=4, fig.width=5}
pc_ds <- parent_data %>% filter(inp_type == "rep") %>% mutate(consecutive_parent_length = as.numeric(key), cnt = value) %>% select(consecutive_parent_length, cnt)

ggplot(pc_ds, aes(consecutive_parent_length, cnt)) + geom_point() + geom_smooth(se=FALSE) + scale_y_log10() + labs(subtitle = "Consecutive parent call counts for input with repeats", x = "Lenght of consecutive parent call sequence", y = "Log10(count)")
rm(pc_ds, parent_data, p)
```

## Consecutive wl calls (MS construction)
```{r, echo=FALSE, message=FALSE, fig.height=4, fig.width=5}
wl_data <- (read_ds(stats_path, TRUE) %>% 
...
```{r, echo=FALSE, message=FALSE, fig.height=4, fig.width=5}
wl_ds <- wl_data %>% filter(inp_type == "rep") %>% mutate(consecutive_wl_length = as.numeric(key), cnt = value) %>% select(consecutive_wl_length, cnt)

ggplot(wl_ds, aes(consecutive_wl_length, cnt)) + geom_point() + geom_smooth(se=FALSE) + scale_y_log10() + labs(subtitle = "Consecutive parent call counts for input with repeats", x = "Lenght of consecutive parent call sequence", y = "Log10(count)")
rm(p, wl_data, wl_ds)
```
...
## Other stats
```{r, echo=FALSE, message=FALSE, fig.height=4, fig.width=5}
i_data <- (read_ds(stats_path, TRUE) %>% 
               filter(measuring == "interval_width"))

kable(i_data %>% 
        spread(key = key, value = value) %>% 
        select(b_path, small, large) %>% 
        mutate(small_perc = round(100 * small / (small + large), 2)) %>% 
        arrange(small_perc), 
      caption="Interval width for various input types.")

rm(i_data)
```

```{r, echo=FALSE, message=FALSE, fig.height=4, fig.width=5}
vcomp_data <- (read_ds(stats_path, TRUE) %>% 
                 filter(measuring == "vector_composition", where=="runs") %>%
                 mutate(vector_value = ifelse(key == 1, "one", "zero")))

kable(vcomp_data %>% 
        select(vector_value, value, inp_type) %>% 
        spread(key = vector_value, value = value) %>% 
        mutate(zero_perc = round(100 * zero / (zero + one), 2)), 
      caption="Composition of the `runs` vector for various input types.")
rm(vcomp_data)
```


```{r, echo=FALSE, message=FALSE, fig.height=4, fig.width=5}
maxrep_data <- (read_ds(stats_path, TRUE) %>% 
                  filter(measuring == "vector_composition", where=="maxrep"))

kable(maxrep_data %>% 
        select(key, value, inp_type) %>% 
        spread(key = key, value = value) %>% 
        mutate(maximal_perc = round(100 * maximal / (non_maximal + maximal), 2)),
      caption="Composition of the `B` vector (containing ends of maximal repeats) for various input types.")
...
len_s <- ds$len_s[1] / 10^6
len_t <- ds$len_t[1] / 10^6

kable(filter(ds, item == "total_time") %>% select(label, value) %>% 
        ...
        arrange(total_s), 
      caption=sprintf("Run time in seconds, on random input with |s| = %dMB, |t| = %dMB", len_s, len_t))
...
ggplot(rbind(fc, od), aes(item, time_ms, color = label)) + geom_boxplot() + geom_jitter(alpha=0.3)
```

\newpage
## Weiner Link optimization -- single vs. double rank
###Sandbox performance
...
```{r, echo=FALSE, message=FALSE}
dds <- read_ds('../single_rank_vs_double_rank/rank_timing.csv', TRUE) %>% filter(measuring == "time", item == "ms_bvector") %>% mutate(time_ms = 100000 * value / len_t)
...
ds <- (read_ds('../use_maxrepeat/sandbox/rank_timing.csv') %>% 
         ...
         select(method, wl_presence, i_width, time_ms, char, maximality) %>% filter(time_ms < 200))
...
## Full Algorithm Performance
```{r, echo=FALSE, message=FALSE}
ds <- (read_ds('../use_maxrepeat/res.small.csv') %>% 
         filter(measuring == "time") %>% 
         filter(measuring == "time", item == "ms_bvector") %>% 
         mutate(time_ms = 100000 * value / len_t)
       )
...
sandbox_ds <- (read_ds('../use_maxrepeat/sandbox/rank_timing.csv') %>% 
                 ...
                 mutate(wl_presence = ifelse(has_wl, "has_wl", "no_wl"), 
                        i_width = ifelse(close, "same_block", "different_block"),
                        maximality = ifelse(is_maximal, "maxrep", "nomaxrep")) %>%
                 mutate(key = node_prop(i_width, maximality)) %>% 
                 select(method, wl_presence, key, char, time_ms) %>% 
                 group_by(method, key) %>% 
                 summarise(time_ms = mean(time_ms)))

comp_ds <- (read_ds("../input_stats_data/rep_100Ms_5Mt.csv") %>% 
              filter(measuring == "maximal_visits") %>% select(key, value))

sandbox_ds <- (sandbox_ds %>% spread(key = method, value = time_ms) %>% 
  inner_join(comp_ds, on = "key") %>% 
  mutate(rf_time = double_rank_fail * value / 1000, 
         mr_time = double_rank_maxrep * value / 1000,
         diff_time = rf_time - mr_time))
sandbox_ds

perf_ds <- (read_ds('../use_maxrepeat/res.csv') %>% 
              filter(measuring == "time", 
                     item == "ms_bvector", 
                     b_path == "rep_100Ms_5Mt") %>% 
              group_by(label) %>% summarise(value = mean(value)) %>% 
              select(label, value)
            )

spread(perf_ds, key = label, value = value) %>% mutate(diff_t = double_rank - double_rank_maxrep)

...
ds <- (read_ds('../use_maxrepeat/sandbox/rank_timing.csv') %>% 
         ...
         select(method, wl_presence, interval_width, time_ms, char, maximality) %>% filter(char != 't'))

kable(spread(ds %>% group_by(method, interval_width, wl_presence, maximality) %>% summarise(avg_time = mean(time_ms)), key = method, value = avg_time),
      digits = 2, caption = "Sandbox performance of the two tricks")

ggplot(ds %>% filter(maximality != "_maxrep"), aes(method, time_ms, color=interval_width)) + 
  geom_boxplot(outlier.shape = NA) + 
  geom_jitter(shape=1, alpha=0.5, width=0.1, size = 3) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1), legend.position="bottom") +
  facet_wrap(maximality~wl_presence, scales = "free_y") +
  labs(title = "Sandbox test") + ylab('milliseconds')
```
...
kable(ds %>% filter(item == "ms_maxrep", label == "double_rank_maxrep") %>% group_by(label) %>% summarize(avg_time_sec = round(mean(value / 1000)), se = round(sd(value / 100))),

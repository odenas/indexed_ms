...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height=4, fig.width=5}
drf_data <- read_ds(stats_path) %>% filter(measuring == "double_rank_niter")
drf_data$b_path <- simplify2array(lapply(strsplit(drf_data$b_path, "_", fixed = TRUE), parse_b_path))

kable(dcast(drf_data %>% select(b_path, key, value), b_path~key)  %>% mutate(perc = round(100 * fail / nofail, 2)) %>% arrange(perc), caption="Double rank iterations that fail for various input types.")
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.height=4, fig.width=5}
maxrep_data <- read_ds(stats_path) %>% filter(measuring == "vector_composition", where=="maxrep")
maxrep_data$b_path <- simplify2array(lapply(strsplit(maxrep_data$b_path, "_", fixed = TRUE), parse_b_path))

kable(dcast(maxrep_data %>% mutate(vector_value = key) %>% select(vector_value, value, b_path), vector_value ~ b_path, value.var="value"), caption="Composition of the `B` vector (containing ends of maximal repeats) for various input types.")
...
#ds <- read_ds('../single_rank_vs_double_rank/rank_timing.csv') %>% filter(item != "dstruct")
#ds
#ggplot(ds, aes(item, time_ms)) + geom_boxplot() + geom_jitter(alpha=0.3)
```
...
ntrials <- 8
```
The figure below shows `r ntrials` runs of the program with and without the use of the `maxrep` (or `B`) vector. Times (in seconds) are shown for the construction of the `ms` bitvector with and without the time for the construction of the `maxrep` bit vector. The input data is random and has |s|=`r sprintf("%dMB", ds$len_s[1] / 10^6)` and |t|=`r sprintf("%dMB", ds$len_t[1] / 10^6)`.

```{r, echo=FALSE, warning=FALSE, message=FALSE}

ds1 <- (ds %>% filter(item == "ms_bvector") %>% select(label, value, item) %>% mutate(item = "ms_bvector_with_maxrep"))

ds2 <- rbind(ds %>% filter(item == "ms_bvector", label == "double_rank") %>% mutate(item = "ms_bvector_without_maxrep") %>% select(item, value, label),
      data.frame(label = "double_rank_maxrep", item = "ms_bvector_without_maxrep",
           value = (ds %>% filter(item == "ms_bvector", label == "double_rank_maxrep") %>% select(item, value, label))$value - 
             (ds %>% filter(item == "ms_maxrep", label == "double_rank_maxrep") %>% select(item, value, label))$value))

ggplot(rbind(ds1, ds2) %>% mutate(time_sec = value / 1000), aes(label, time_sec)) + geom_boxplot() + geom_jitter(alpha=0.5) + facet_wrap(~item)
```
...
rank_comp <- dcast(dd, item+b_path~label, value.var="avg_time") %>% mutate(abs_ratio = double_rank_and_fail / double_rank, rel_ratio = 100 * abs(double_rank_and_fail - double_rank) / double_rank)
kable(data.frame(rank_comp),
      digits=2, caption="Single vs. double rank. Absolute (double / single) and relative (100 * |double - single| / single) ratios of average times.", row.names=FALSE)
...
bb$b_path <- simplify2array(lapply(strsplit(bb$b_path, "_", fixed = TRUE), parse_b_path))

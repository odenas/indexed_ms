...
big_dt <- do.call(rbind, lapply(c(alps, alpl), function(a){
  ...
  read_csv(p) %>% mutate(alp = ifelse(a==alps, "small", "large"))
})) %>% mutate(dset="big")
...
all_dt <- (rbind(big_dt, small_dt) %>% 
             mutate(tcall = t_micro / ncalls, 
                    ...
                    has_wl=ifelse(has_wl == 1, "wl", "nowl"),
                    node=ifelse(node == 1, "internal", "leaf")) %>% 
...
ggplot(all_dt, aes(tp, tcall, fill=method)) + geom_boxplot(position='dodge') + facet_grid(alp~dset, scales = "free_y")

library(ggridges)

ggplot(all_dt %>% filter(dset=="small", alp=="small"), aes(x = tcall, y = method)) + 
  geom_density_ridges() + facet_wrap(~tp, scales='free', ncol = 2)

ggplot(all_dt %>% filter(dset=="big", alp=="small"), aes(x = tcall, y = method)) + 
...
all_dt <- (read_csv("../parent_tests/sandbox_parent/all.csv") %>% 
             ...
             select(-other, -st, -sl, -tt, -tl))

ggplot(all_dt %>% filter(alp=='abcd'), aes(parent_depth, value/nwd_cnt, group=method)) + geom_line(alpha=0.3) + geom_point(aes(color=nwd_cnt))

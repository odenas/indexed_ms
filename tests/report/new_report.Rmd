...
input_stats_data <- (read_csv('../input_stats_data/stats.csv') %>% 
                       separate(b_path, into=c('stype', 'slen', 
                                               'ttype', 'tlen', 
                                               'alp', 'ext')) %>% 
...
dd <- all_dt %>% filter(alp=='abcd', sl=="big", nwd_cnt > 200)
...
```{r expected_times}
fit.pseq <- lm(value ~ parent_depth, all_dt %>% filter(sl=='big', method=='pseq', nwd_cnt > 50))
fit.lca <- lm(value ~ parent_depth, all_dt %>% filter(sl=='big', method!='pseq', nwd_cnt > 50))

ggplot(all_dt %>% filter(sl=='big', method!='pseq', nwd_cnt > 250), aes(parent_depth, value)) + geom_line() + geom_point()


#pdata %>% group_by(inp_type, alp) %>% summarise(exp_time = value %*% predict(fit, data.frame(parent_depth=key)))


dd <- all_dt %>% filter(sl=="big") %>% select(-sl)
p_stats <- pdata %>% mutate(parent_depth = key, cnt = value) %>% 
  select(-key, -value)
expected_data <- (left_join(p_stats, dd, by = c("parent_depth", "alp")) %>% 
                    mutate(expected_time = cnt * value) %>%
                    filter(!is.na(method)))
...
expl_plot_data <- (expected_data %>% 
    group_by(inp_type, alp, method) %>% 
    summarise(total_exp_time = sum(expected_time)) %>% 
    spread(method, total_exp_time) %>% 
    mutate(rel_diff=abs(lca-pseq) / pmax(lca, pseq))
)
ggplot(expl_plot_data, aes(inp_type, rel_diff)) + geom_point() + geom_hline(yintercept = 0) + ylim(-1, 1) + facet_wrap(~alp)
```


## full tests
```{r parent_full, eval=FALSE}

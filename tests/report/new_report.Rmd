...
run the following command (makefile snippet):
...
## sandbox tests
```{r m0c0_zoom, eval=FALSE}

all_dt <- do.call(rbind, lapply(1:5, function(i){
  path <- sprintf('../wl_tests/sandbox_maxrep/rep_1000000s_sim_200000t_abcd.%d.csv', i)
  read_csv(path) %>% mutate(ntrial=i, 
                            maximal=ifelse(maximal == 1, "maximal", "non-maximal"),
                            has_wl=ifelse(has_wl == 1, "has_wl", "no_wl"),
                            )
} )) #%>% unite(tp, maximal, has_wl)

#ggplot(all_dt, aes(tp, t_micro/1000, fill=method)) + geom_boxplot(position='dodge')
ggplot(all_dt, aes(method, t_micro/1000)) + geom_boxplot(position='dodge') + facet_grid(maximal~has_wl, scales = "free_y")
```

```{r maximal0_close1}
pp <- c(0, 1)
all_dt <- readRDS(sprintf('../wl_tests/sandbox_maxrep/maximal%d_close%d.rds', pp[1], pp[2]))
all_dt
ggplot(data = all_dt %>% filter(time_micro < 10), mapping = aes(method, time_micro)) + 
  geom_boxplot(outlier.shape = NA)  + 
  labs(title = "Non-maximal", subtitle = "small interval") + facet_wrap(inp_type~alp)
```
...
ggplot(data = all_dt %>% filter(time_micro < 10), mapping = aes(method, time_micro)) + 
  geom_boxplot(outlier.shape = NA) + 
  labs(title = "Non-maximal", subtitle = "large interval") + facet_wrap(inp_type~alp)
```
...
#ggplot(all_dt, aes(factor(has_wl), time_micro)) + geom_boxplot() + facet_grid(method~inp_type, scales="free")

#ggplot(all_dt %>% filter(alp!="abcd", method=="s", time_micro < 20), aes(time_micro)) + geom_histogram(binwidth = 1) + 
#  facet_grid(method~inp_type, scales="free") + 
#  labs(subtitle = "in-box distribution (large alphabet)")

#summary(all_dt)

ggplot(data = all_dt %>% filter(time_micro < 20), mapping = aes(method, time_micro)) + 
  geom_boxplot(outlier.shape = NA) + 
  labs(title = "Maximal", subtitle = "small interval") + facet_wrap(inp_type~alp)
```
...
ggplot(data = all_dt %>% filter(time_micro < 20), mapping = aes(method, time_micro)) + 
  geom_boxplot(outlier.shape = NA) + 
  labs(title = "Maximal", subtitle = "large interval") + facet_wrap(inp_type~alp)
```
...
```{r fulltest_data}
time_data <- (read_csv(sprintf('../wl_tests/rank_timing.csv', ".")) %>% 
                filter(measuring == "time", item == "ms_bvector") %>% 
                separate(b_path, into=c('stype', 'slen', 'ttype', 'tlen', 'alp', 'mp')) %>% 
                separate(label, into=c("lazy", "fail", "maxrep")) %>% 
                unite(col = inp_type, stype, ttype, sep = "_") %>% 
...
prange_plot(dt %>% 
              select(lazy, ntrial, alp, inp_type, value) %>% spread(key = lazy, value = value) %>%
...
prange_plot(dt %>% 
              select(maxrep, ntrial, alp, inp_type, value) %>% spread(key = maxrep, value = value) %>% 
              group_by(alp, inp_type) %>% 
...
sdd <- readRDS('../parent_tests/sandbox_selectatdist/group_data.rds') %>% filter(dist < 150)
ggplot(data=sdd, aes(x=dist, y=time_avg, colour=method)) + geom_line() + geom_ribbon(aes(ymin=time_avg - time_sd, ymax=time_avg + time_sd, fill=method), linetype=0, alpha=0.1) + facet_grid(alp~inp_type, scales = "free_y")
```
...
time_data <- (read_csv("../parent_tests/rank_timing.csv") %>% 
                ...
                separate(b_path, into=c('stype', 'slen', 'ttype', 'tlen', 'alp', 'mp')) %>% 
                unite(col = inp_type, stype, ttype, sep = "_") %>% 
                spread(key = item, value = value) %>% select(ntrial, label, inp_type, alp, ms_bvector))
#time_data

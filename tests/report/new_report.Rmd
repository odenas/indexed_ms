...
$$
d^{(i)} = \frac{t^{(i)}_{\mathtt{opt}} - \bar{t}_{\mathtt{non_opt}}}{\max\{t^{(i)}_{\mathtt{opt}}, \bar{t}_{\mathtt{non_opt}}\}}
$$
with $\bar{t}_{\mathtt{non_opt}} = 1/n \sum t^{(i)}_{\mathtt{non_opt}}$. Hence negative values indicate a speedup by the optimization over the average non-optimized time: -0.5 is a 2x speedup etc, in general the speed up is $-1/d^{(i)}$. 
...
```{r, echo=FALSE}
time_data <- (read_csv('../parallelization_data/./stats.csv') %>% 

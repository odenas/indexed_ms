...
ggplot(dd %>% mutate(call_volume = ifelse(parent_depth < 6, "first_parent_depths", "other_parent_depths")), aes(parent_depth, nwd_cnt)) + geom_line() + geom_point() + facet_wrap(~call_volume, scales ="free")
...
dd <- all_dt %>% filter(alp!='abcd', sl=="big", parent_depth < 15)
tt <- "Big(20) alphabet, big index(100Mb)"
ggplot(dd, aes(parent_depth, value/nwd_cnt, color=method)) + geom_line() + geom_point() + labs(subtitle = tt)
ggplot(dd %>% mutate(call_volume = ifelse(parent_depth < 4, "first_parent_depths", "other_parent_depths")), aes(parent_depth, nwd_cnt)) + geom_line() + geom_point() + facet_wrap(~call_volume, scales ="free")
```
...
```{r expected_times}
dd <- all_dt %>% filter(sl=="big", parent_depth < 10)
p_stats <- (read_csv('../parent_tests/stats.csv') %>% 
              separate(fname,  c('st', 'sl', 'tt', 'tl', 'alp', 'other', 'other1')) %>% unite(inp_type, st, tt) %>% 
              mutate(parent_depth = seq_len) %>% select(-seq_len) %>%
              select(-other, -other1, -sl, -tl) %>% 
              group_by(parent_depth, inp_type, alp) %>% 
...
** FLAGS ARE INVERTED, WHAT YOU SEE IS A SLOWDOWN **

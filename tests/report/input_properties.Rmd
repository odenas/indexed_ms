...
pdata <- (all_data %>% 
            filter(measuring == "pseq", where == "ms") %>% mutate(key = as.numeric(key)) %>% 
 ...
 * s_type: The index (aka $S$) type. 
      - `rep`  the index is the concatenation of $(s_1, \ldots, s_k)$ where  $\sum_i |s_i| = |S|$.
 * t_type: The query (aka $T$) type
      - `sim` similar to the index, up to a certain number of mutations `sim_mut` 
      - `dis` randomly generated
 * slen/tlen: length of the index/query string
 * alp: the alphabet
 * rep_mut: number of mutations introduced to block $s_1$ to obtain blocks $s_i$ with $1 < i \leq n$.
 * sim_mut: number of mutations introduced to $S$ in order to obtain $T$ -- I.e., $T$ differs from $S$ by at most `sim_mut` characters.
...
Notice that the row with `rep_mut=rep_sim=1` has `S` that is a almost a perfect repeat of blocks, and `T` almost a perfect copy of it. On the other hand, `rep_mut=rep_sim=1` has `S` pretty much random (since we are  mutating all chars of a block) and `T` having mismatches everywhere.
...
for(i in c(1, 10, 100)){
  print(ggplot(wldata %>% filter(rep_mut==i), aes(key, value, group=sim_mut, color=sim_mut)) + 
    geom_point() + geom_line(alpha=0.2) + 
    facet_wrap(~alp, ncol = 1) + scale_y_log10() +
...
for(i in c(1, 10, 100)){
  plot(ggplot(pdata %>% filter(rep_mut==i), aes(as.numeric(key), value, group=sim_mut, color=sim_mut)) + 
         geom_point() + geom_line(alpha=0.2) +
         facet_wrap(~alp, ncol=1) + scale_y_log10() +
         labs(title = sprintf('rep_mut = %d', i), x=xl, y="log10(count)"))
}

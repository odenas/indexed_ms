...
from utils import MsInterface, verbose_args, MsInput
...
def main(opt):
    ...
    base_dict = dict(load_cst=opt.load_cst, rank_fail=opt.rank_fail,
    ...
    for i, pref in enumerate(opt.prefixes):
        ...
        for j in range(opt.repeat):
            with open(opt.output, 'a') as fd:
                ...
                if i + j == 0:
                    fd.write(res[0] + ",label,nrep,b_path\n")
                for line in res[1:]:

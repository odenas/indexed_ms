...
wlink_argument(const t_cst &cst, const typename t_cst::node_type v) {
    std::uniform_int_distribution<typename t_cst::size_type> distribution(cst.lb(v), cst.rb(v));
...
template<class t_cst>
void test_wlink(const t_cst &cst, const std::pair<typename t_cst::node_type, typename t_cst::char_type> &arg) {
    cst.wl(arg.first, arg.second);
}
...
void run_benchmark(std::string cst_name, const t_cst &cst) {
    ...
    run_benchmark(cst_name, cst, "WLINK", test_wlink<t_cst>, wlink_argument<t_cst>);

...
read_time_dataset <- function(input_id){
  time_dd <- (
    read_csv(sprintf("../range_tests/%s.t.ms.csv", input_id))
    %>% filter(method %in% c('algorithm'))
    %>% mutate(time_ms = time_ms / nqueries)
    %>% select(-method, -nqueries)
  )
...
read_sizes_dataset <- function(input_id){
  space_dd <- (
    ...
    %>% mutate(size = sapply(compression, function(ext) file.info(sprintf('../range_tests/%s.t.ms.%s', input_id, ext))$size))
  )
...
ggplot(time_dd
       %>% inner_join(space_dd, by=c("compression", "dset"))
       %>% filter(compression != "_none"),

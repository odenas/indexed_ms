...
ggplot(dd %>% filter(action %in% c('correct', 'vec')) %>% spread(action, avg), aes(correct, vec)) + geom_point() + geom_smooth() + facet_wrap(~inp_type)
...
ggplot(dd %>% filter(section != 'comp'), aes(nthreads, avg, color=action)) + geom_line() + geom_point() + facet_grid(section~inp_type, scales = 'free_x')

ggplot(dd %>% filter(section == 'comp'), aes(nthreads, mid)) + geom_line() + geom_point() + facet_wrap(inp_type~., scales = 'free_x')
```

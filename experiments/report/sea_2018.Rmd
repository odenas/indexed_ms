...
p1 <- ggplot(df1, aes(nthreads, log2(time_ratio), color = inp_type)) +
  ...
  scale_y_continuous(name = "Time ratio (log scale)") +
  #theme(legend.position = "none") +
  labs(subtitle = 'Runs build')
p1
...
q1 <- ggplot(df1 %>% filter(nthreads > 1),
  ...
  scale_y_continuous(name = "Rel. time ratio", limits = c(-0.5, 1.5)) +
  labs(subtitle = 'Runs build')
q1
...
p2 <- ggplot(df2, aes(nthreads, time_ratio, color = inp_type)) +
  geom_jitter(width = 0.05, shape=1) +
  theme(legend.position = "none") +
  scale_x_continuous(trans = 'log2', breaks = n_threads_original, name = "") + 
  scale_y_continuous(name = "Fraction of total") +
  labs(subtitle = 'Runs correction')
p2

q2 <- ggplot(df2 %>% filter(inp_type != 'individ-same'), aes(nthreads, time_ratio, color = inp_type)) +
  geom_jitter(width = 0.05, shape=1) +
  theme(legend.position = "none") +
  scale_x_continuous(trans = 'log2', breaks = n_threads_original, name = "N. threads") +
  scale_y_continuous(name = "Fraction of total") +
  labs(subtitle = "Runs correction (zoom)")
q2


# third column
df3 <- (df %>% filter(name == 'ms_build')
       %>% left_join((df
                      %>% filter(nthreads == 1)
                      %>% select(-nthreads)
                      %>% rename(thr1_time_ms = time_ms)),
...
p4 <- ggplot(df4, aes(nthreads, log2(time_ratio), color = inp_type)) +
  ...
  labs(subtitle = "Total")
p4
q4 <- ggplot(df4 %>% filter(nthreads > 1, inp_type != 'individ-same'),
             aes(nthreads, (time_ratio - ideal_time_ratio) / ideal_time_ratio, color = inp_type)) +
  geom_jitter(width = 0.05, shape=1) + 
  theme(legend.position = "none") +
  scale_x_continuous(trans = 'log2', breaks = n_threads_original, name = "N. threads") +

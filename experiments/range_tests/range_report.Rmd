...
dd <- read_time_dataset('mm') %>% filter(compression=='none', block_size != 1e6) %>% select(-compression)
dd
ggplot(dd, aes(range_size, time_ms, color=factor(block_size))) + geom_line() + geom_point() + scale_x_log10()

...
dd <- (
  ...
  %>% select(-compression, -method, -nqueries)
  %>% mutate(uses_block = block_size < range_size)
)
dd
ggplot(dd %>% filter(range_size > 0), aes(range_size, time_ms, color=factor(block_size))) +
  geom_line() + geom_point() +
  scale_x_log10() + scale_y_log10() + expand_limits(0, 0) +
  facet_wrap(~dset)


dd <- (
  rbind(read_time_dataset('HG03061-2'), read_time_dataset('mm'))
  %>% filter(algo == 't', compression=='none') %>% select(-compression, -algo)

...
dd <- (
  ...
  %>% select(-nqueries, -method)
)

ggplot(dd, aes(block_size, time_ms, color=compression, linetype=algo, shape=algo)) +
  geom_line() + geom_point() + 
  scale_x_log10() + scale_y_log10() + expand_limits(y=c(0, 1)) +
  facet_wrap(~dset) + labs(subtitle = "waiting on results...")
```
...
strings of `0`s are counted in the next block). Then the answer is
obtained by the sum of block sums plus the sum of values in the 
blocks spanning the interval endpoints.
...
of the input range in the bit vector. 
...
interval and keep a current max. This approach needs to issue 2 select queries in `ms`
and a linear time scan of the corresponding bit vector portion. 
...
ggplot(time_dd, aes(range_size, time_ms, group=factor(block_size))) +
  ...
  geom_text(data = size_dd %>% mutate(a=1e+5 + runif(nrow(size_dd))/2),
            aes(brsize, a, label=format(round(brsize, 0), big.mark = "'", scientific = TRUE), angle=90)) +
  scale_x_log10() + scale_y_log10() + expand_limits(y=c(0, 1)) +
  facet_wrap(~dset) + labs(subtitle = "trivial algorithm, uncompressed")
```

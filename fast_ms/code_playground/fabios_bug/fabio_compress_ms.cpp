...
size_type compute_length(const size_type n_ones, const size_type n_zeros){
    // this will support the case when n_zeros==0
    if (n_ones < n_zeros)
...
size_type fill_encoder_runCompression(sdsl::bit_vector ms, enc_type& encoder) {
    size_type i, nBitsInOutput, nOnes, nZeros, length, nRuns;
...
//    printf("nZeros=%lu, nOnes=%lu, length=%lu\n",nZeros,nOnes,length);        
    if (nOnes>0) {
        nRuns++;
        length = compute_length(nOnes, nZeros);
        printf("nZeros=%lu, nOnes=%lu, length=%lu\n",nZeros,nOnes,length);        
...
int main(int argc, char **argv){
    ...
    if(argc == 1){
        (cerr << "Compress a ms vector. Creates files <ms_path>.xxx\n"
              ...
              << endl);
        //exit(0);
        ms_path = "q_influenza-1-39020_t_influenza-39021-78041.ms";
        flags.compression = ms_compression::parse_compression("rle");
    } else {
        try{

...
public:
    ...
    InputFlags(const bool load_cst_, const bool load_maxrep_){
        load_cst = load_cst_;
        load_maxrep = load_maxrep_;
    }

    InputFlags(const InputFlags& i) {
        load_cst = i.load_cst;
        load_maxrep = i.load_maxrep;
    }
...
void build_ms(const string& t, const StreeOhleb<>& st, sdsl::bit_vector& runs, sdsl::bit_vector& ms, sdsl::bit_vector& maxrep,
    ...
    while(k < ms_size){
        ...
        k_prim = MsVectors<StreeOhleb<>, sdsl::bit_vector>::find_k_prim_(k, ms_size, runs);
...
void comp(InputSpec& T, InputSpec& S_fwd, const string& out_path, InputFlags& flags){
    ...
    /* build stree */
    StreeOhleb<> st;
    size_type load_cst_time = load_or_build(st, s, S_fwd.fwd_cst_fname, flags.load_cst);
    ...
    /* build the cst */
    load_cst_time = load_or_build(st, s, S_fwd.rev_cst_fname, flags.load_cst);
    cerr << "DONE(" << load_cst_time << "milliseconds)" << endl;

    /* build the maxrep vector */
    Maxrep<StreeOhleb<>, sdsl::bit_vector> maxrep_obj{};
    size_type time_usage_ms_maxrep = Maxrep<StreeOhleb<>, sdsl::bit_vector>::load_or_build(maxrep_obj, st, S_fwd.rev_maxrep_fname, flags.load_maxrep);

...
public:
    ...
    wl_method_t1 get_wl_method() {
        if(double_rank){
            if(lazy){
                return (rank_fail ? &cst_t::lazy_double_rank_fail_wl : &cst_t::lazy_double_rank_nofail_wl);
            }
            return (rank_fail ? &cst_t::double_rank_fail_wl : &cst_t::double_rank_nofail_wl);
        } else {
            return (lazy ? &cst_t::lazy_single_rank_wl : &cst_t::single_rank_wl);
        }
    }

    wl_method_t2 get_mrep_wl_method() {
        return (use_maxrep_rc ? &cst_t::double_rank_fail_wl_mrep_rc: &cst_t::double_rank_fail_wl_mrep_vanilla);
    }
...
void comp(const InputSpec& tspec, InputSpec& s_fwd, Counter<size_type>& time_usage, InputFlags& flags){

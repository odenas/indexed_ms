...
//--------------------------------------------------------------------------
    ...
    class Iterator  // FC> just offers sequential access to the samples.
    {
      public:
      ...
      protected:
        ...
        usint      block;  // FC> sample/block ID
        pair_type  sample;
        usint      cur; // cur == 0 is the sample  FC> ???????????
		usint      val; // FC> position
		usint      run; // FC> ?????????????
        usint      block_items;  // FC> number of 1-bits in the block
        ...
        */
        ...
        inline void getSample(usint sample_number)  // FC> just a loading function
        {
          this->block = sample_number;
          this->samples.goToItem(2 * sample_number);
          this->sample.first = this->samples.readItem();  // FC> rank
          this->sample.second = this->samples.readItem();  // FC> position of the first 1-bit in the block
          this->cur = 0;
...
//--------------------------------------------------------------------------
  ...
  protected:
    ...
    const usint* array;  // FC> stores the actual bits
    usint        block_size;
    ...
    */
    ...
    ReadBuffer*  rank_index;  // FC> mutiple bitvector samples occur inside the same rank_rate block
    usint        rank_rate;

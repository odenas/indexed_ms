...
class RLEEncoder : public VectorEncoder
{
  public:
	...
	const usint THRESHOLD = 0; //=Always use diff encoding.   //1e9;
...
//printf("%lu,%lu,%lu \n",diff,len,project(diff,len,1,THRESHOLD));
	  this->buffer->writeDeltaCode(project(diff,len,1,THRESHOLD));  // FC> Remark: $writeDeltaCode()$ assumes positive input.
    }

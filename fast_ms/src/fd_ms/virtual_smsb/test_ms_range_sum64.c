...
static uint64_t range_ms_sum_fast_in_blocks64(uint64_t first_ms, uint64_t first_bit_idx, uint64_t last_bit_idx,uint64_t * bitvec)
{
...
//    first_select_res = 61, last_select_res = 68;
	...
	for(i=0;i<nqueries;i++)
	{
		...
		range_sum2=naive_range_ms64(first_idx,last_idx,nbits,bitvec);
		range_ms_sum_fast_ext64(first_ms,first_select_res+1,
			last_select_res,bitvec,&virtual_ms,&range_sum3);
		range_sum3+=first_ms;
		range_sum4=range_ms_sum_fast_in_blocks64(first_ms,first_select_res,
			last_select_res,bitvec);

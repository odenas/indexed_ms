...
void
BitVector::indexForRank()  // FC> after every $rank_rate$ positions, stores the block of the bitvector in which you are. computed by scanning $samples$.
{
  delete this->rank_index;
...
void
BitVector::indexForSelect()  // FC> after every $select_rate$ ones, stores the block of the bitvector in which you are. computed by scanning $samples$.
{
  delete this->select_index;
...
usint
BitVector::Iterator::sampleForValue(usint value)  // FC> map (position in bitvector) -> (sampling block that contains it)
{

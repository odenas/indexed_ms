 ...
 * encoding, where each $nZeros_x$ and $nOnes_x$ is delta-coded separately. The minimal 
 * encoding is equivalent to a shortest path in the DAG: the program computes all shortest
 * paths in order to answer multiple queries later.
 */
#include <stdint.h>
#include <stdlib.h>
#include <stdio.h>
...
/**
 * @return the number of bits for encoding the run pair $(nZeros,nOnes)$.
 */
 ...
 * threshold
 * intitialMSValue_first (< threshold)
 * intitialMSValue_last (< threshold)
 * maxNZeros 
 * maxNOnes 
 * allowNegativeMS
...
int main(int argc, char **argv){
	...
	for (i=INITIAL_MS_VALUE_FROM; i<=INITIAL_MS_VALUE_TO; i++) {
		...
		clean();
		buildDAG(i);
		sprintf(buffer,"%s/table-diffLong-%u-%u-%lu-%lu-%u",DIRECTORY,THRESHOLD,i,N_ZEROS,N_ONES,ALLOW_NEGATIVE_MS);
		file=fopen(buffer,"w");
